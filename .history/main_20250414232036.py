...
class Stock:
    def __init__(self, ticker, quantity, buy_price):
        ...
        self.ticker = ticker.upper()  # Ensure the ticker symbol is in uppercase
...
class Portfolio:
    ...
    def add_stock(self, ticker, quantity, buy_price):
        ...
        ticker = self.entry_ticker.get().strip().upper().replace("$", "")
...
class Dashboard(tk.Frame):
    ...
    def update_portfolio_table(self):
        ...
        for stock in self.portfolio.stocks:
            ...
            self.table.insert("", "end", values=(stock.company_name, stock.ticker, stock.quantity, stock.buy_price, round(stock.current_price, 2)))
